...
# ==========================================
async def retrieve_from_mongo(query_embedding: List[float], limit: int = 3, num_candidates_multiplier: int = 10) -> List[Dict]:
    """
    ...
    Args:
        ...
        num_candidates_multiplier (int, optional): Multiplier for numCandidates in vector search. Defaults to 10.
...
# ==========================================
async def retrieve_documents(state: GraphState, config: RunnableConfig, num_candidates_multiplier: int = 10) -> Dict:
    """Node: Vector search. Strictly populates state with pointers (IDs)."""
    emit_log(config, "--- NODE: RETRIEVE DOCUMENTS ---")
    query = state["query"]
    query_embedding = await embeddings.aembed_query(query)
    docs = await retrieve_from_mongo(query_embedding, limit=10, num_candidates_multiplier=num_candidates_multiplier)
